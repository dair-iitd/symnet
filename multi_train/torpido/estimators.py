    ...
    def __init__(self,
        ...
        with tf.variable_scope("policy_net", reuse=tf.AUTO_REUSE):
            ...
            for env_i in range(self.N):
                ...
                action_embedding2 = tf.layers.dense(
                    inputs=self.graph_embedding_list[env_i],
                    units=self.num_decoder_dim,
                    activation=self.activation_fn,
                    name="action_embedding2_{}".format(env_i),
                    reuse=tf.AUTO_REUSE)

                action_embedding3 = tf.layers.dense(
                    inputs=action_embedding2,
                    units=self.num_valid_actions_list[env_i],
                    activation=self.activation_fn,
                    name="action_embedding3_{}".format(env_i),
                    reuse=tf.AUTO_REUSE)

                self.predicted_action_list[env_i] = action_embedding3
