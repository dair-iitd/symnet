...
# for i in range(2, 4):
#     for j in range(num_type_actions):
#         vars_to_load += tf.get_collection(
#             tf.GraphKeys.GLOBAL_VARIABLES,
#             scope='global/policy_net/action_embedding{}_{}'.format(i, j))

vars_to_load = tf.get_collection(
    tf.GraphKeys.GLOBAL_VARIABLES, scope='global/policy_net')

vars_to_load += tf.get_collection(
    tf.GraphKeys.GLOBAL_VARIABLES, scope='global/value_net')

for worker_id in range(NUM_WORKERS):
    vars_to_load += tf.get_collection(
        tf.GraphKeys.GLOBAL_VARIABLES,
        scope='worker_{}/value_net'.format(worker_id))

    vars_to_load += tf.get_collection(
        tf.GraphKeys.GLOBAL_VARIABLES,
        scope='worker_{}/policy_net'.format(worker_id))

vars_to_load += tf.get_collection(
    tf.GraphKeys.GLOBAL_VARIABLES, scope='RMSProp')
